...
Judge 评分：交互 7.8 / 样式 8.4 / 功能 8.8。P0 为空；当前 P1 是悬浮窗在额度页遮挡图表，以及 TODO 中尚未实现的高价值功能。
...
- P2 状态：刷新成功后状态栏左侧为空，用户看不到数据更新时间与是否来自缓存。
- P2 样式：一年热力图在约 814px 内容宽度下扫读密度偏高，需评估分段或缩放交互。

## 若继续，下一轮会做

1. 数据新鲜度与同步状态（P1）：消除刷新按钮左侧空白，明确最新 / 刷新中 / 缓存回退。
2. 修复悬浮窗在「额度」页遮挡图表（P1），保持其他 view 的常驻能力。
3. 悬浮窗点击直达「额度」tab（P1）：确认 DSH 的 view 切换 API 是否对插件开放。
