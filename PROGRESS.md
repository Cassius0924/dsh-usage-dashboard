...
- （轮次 25）`feat(dashboard)`: 异常消耗侦测。
  - Review / Critique：P1 真实数据中 8 月 14 日费用是日常基线的 6.1 倍，但只能靠目测柱高发现；
    P1 找到高柱后仍不知道哪个模型 / 会话造成；P2 若只用倍数会把低额小波动误报成异常。
    功能缺口扫描选择脑暴 #3 的异常消耗侦测。
  - Act：host 在一次日志回放中保留逐会话日聚合，以此前 7 个自然日的活跃日均费用作基线；只有历史至少 10 次调用、
    当日费用至少 ¥1、达到基线 2.5 倍且绝对多出 ¥1 才记录，最多返回最近 5 项，并归因主要模型 / 会话。
    用量卡显示解释性提示，普通与分模型逐日柱图标出异常日期；无异常时不渲染提示。
  - Verify：33/33 单测通过（含异常归因与低样本不误报）、build、typecheck 全过；重启服务后 active，
    登录后的根页面与 usage API 均为 200。真实数据识别 `2026-08-14`：¥9.21 / 基线 ¥1.50 / 6.1 倍，
    主要模型 `deepseek-v4-pro`，主要会话 ¥8.46（92%）。Playwright 实测提示可见、1 根异常柱、tooltip 含
    日期 / tokens / 费用 / 300 次调用；注入无异常响应后提示节点完全消失，恢复真实响应后重新出现；
    console error 0、pageerror 0。截图：`/tmp/dsh-dashboard-audit/anomaly.png`。
  - Impeccable audit：本轮无新增警告；仍仅有既有 4 处 `transition: width` P3 性能提示。
