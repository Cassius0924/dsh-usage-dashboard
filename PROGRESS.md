...
用户要求继续迭代，轮次上限重置。当前状态：可交付，持续加功能中。
Judge 评分：交互 8.5 / 样式 8.5 / 功能 9.0（缺陷层面 P0、P1 已清空；剩余 P1/P2 均为 TODO 里尚未实现的候选功能，不是缺陷）。
...
1. 月度预算与进度条（P1）：设预算 ¥X，显示已用比例与按当前速率的月底预测。
2. 悬浮窗点击直达「额度」tab（P1）：需先确认 DSH 的 view 切换 API 是否对插件开放。
3. 费用走势折线图 / 导出 CSV / 会话维度成本 Top N（P2）。
