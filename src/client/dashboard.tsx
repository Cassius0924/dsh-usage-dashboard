...
export function BalanceDashboard(): ReactElement {
  ...
  const load = async (): Promise<void> => {
    ...
    try {
      ...
      if (b.ok && b.data !== undefined) setBalance(b.data)
      else setBalance(null)
      if (u.ok && u.data !== undefined) setUsage(u.data)
      else setUsage(null)
      if ((!b.ok) && (!u.ok)) setError(b.error ?? u.error ?? '加载失败')
    } catch (err) {
  ...
  return (
    <div className="dq-balance">
      {loading && <div className="dq-muted">加载中…</div>}
      {error !== null && <div className="dq-error">{error}</div>}
